...
The `browser.test` API will also be used to send messages back to the web page to notify the test harness when a test has started and finished. For these cases, it is up to the user agent to send the messages "test-started" and "test-finished", respectively. These messages should be sent when a test that was passed into `browser.test.runTests` has started and finished. In addition, when recording the result for a `browser.test.assert...()` method, it is up to the user agent to send the messages "assert" or "assert-equality".
